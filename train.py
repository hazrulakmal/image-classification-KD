...
from src.models import LightningModel #PetModel, 
...
class MyLightningCLI(LightningCLI):
    def add_arguments_to_parser(self, parser):
        ...
        #parser.add_argument("--api_key", type=str, default=None, help="Name of the model to use")
        parser.set_defaults({
            "trainer.logger": {
...
def cli_main(args: ArgsType = None):    
    ...
    cli.trainer.fit(model=lightning_model, datamodule=dm)
    cli.trainer.test(model=lightning_model, datamodule=dm, ckpt_path=ckpt_path)
    wandb.finish()
